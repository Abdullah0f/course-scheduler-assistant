...
export function getCourses(htmlString: string): Record<string, Course[]> {
  ...
  rows.forEach((row) => {
    ...
    const open = cells[i++]?.textContent || undefined
...
export function generatePeriods(periodsString: string, type: string | undefined, instructorName: string | undefined): Period[] {
...
function createAnotherCourseWithThisPeriod(course: Course, periods: Period[], type: string|undefined) {
